...
**제품:** Jokguboard  
**정리일:** 2026-09-16  
**현재 설치:** `app-debug-0916_1253-hidnobump.apk`  
**다운로드:** https://jokguboard.vercel.app/downloads/app-debug-0916_1253-hidnobump.apk  
...
| `app-debug-0910_1049-hitdepth.apk` | 10:49 | 사이버 터치·그림자 약 30% | `5518ad6` |
| `app-debug-0910_1118-midfit.apk` | 11:18 | 세트 칸 비율 글자·시각·경기시간 | `57f140e` |
| `app-debug-0910_1145-setmid.apk` | 11:45 | 세트 점수 키움·세로 가운데 모임 | `caa51ae` |
| `app-debug-0910_1303-servemark.apk` | 13:03 | 세트 안쪽 모임·심플 서브 원·화살표 | `01689e1` |
| `app-debug-0910_1326-servebar.apk` | 13:26 | 서브 바·원 확대·가운데 문구 제거 | `29bcc40` |
| `app-debug-0910_1346-midtype.apk` | 13:46 | 서브 바 숫자 붙임·세트·시계 글자 | `3173192` |
...
| `app-debug-0916_1253-hidnobump.apk` | 12:53 | HID +1은 숫자만 (애니 없음) | `3506070` |

## 현재 버전에서 쓰는 것

`app-debug-0916_1253-hidnobump.apk`가 위 표를 모두 이은 최신이다.
...
- 큰 점수는 남은 칸을 채움 (`JK_SCORE_FILL`). 층 그림자 약 30%(`JK_SCORE_DEPTH`). 화면 터치 +1은 숫자 확대와 표식 상승(`JK_SCORE_BUMP`). HID +1은 숫자만(`JK_HID_NO_BUMP`). 사이버·링에서 팀명·점수 터치(`JK_CYBER_HIT`)
- 가운데 칸 글자는 칸 크기에 맞춤(`JK_MID_FIT`). 「세트」 라벨 3배. 시각·경기 시계 세로는 2배·가로는 1.3배. 세트 점수 숫자는 크기 그대로 안쪽. 시각·새 경기는 세트 점수 아래 세로 가운데. 세트 칸에 팀 서브 문구 없음. 심플에서는 가운데 칸 맨 위에 금색 서브 번호(원 안 숫자 1.5배, 화살표 없음)(`JK_SIMPLE`). 서브 팀 점수 숫자 바로 아래 짧은 막대(`JK_SERVE_BAR`, 밝은 백 칸 검정·어두운 칸 흰색, 두께·길이 1.5배). 가운데 칸 폭은 그대로. 현재 시각·경기 시간(선서브부터, 새 경기에서 0). 새 경기 글자는 버튼에 맞춤
...
- 패드 Kotlin은 위 버튼 판정 뒤에만 시스템 바를 닫음. 손가락으로 연 바는 그대로(`JK_HID_BAR_GUARD`). 맞춤이 켜진 동안 3버튼 리모컨 마우스 커서는 숨김(`JK_PAD_POINTER_HIDE`). `barwatch`의 decorView insets 창은 위 득점·커서·바를 깨서 되돌림
- 심플, TTS, BLE, 스킨, 하단 메뉴. 심플 코트 원 숨김, 가운데 칸 맨 위 서브 번호(화살표 없음)
