...
**제품:** Jokguboard  
**정리일:** 2026-09-09  
**현재 설치:** `app-debug-0910_1356-barwhite.apk`  
**다운로드:** https://jokguboard.vercel.app/downloads/app-debug-0910_1356-barwhite.apk  
...
`app-debug-0910_1356-barwhite.apk`가 위 표를 모두 이은 최신이다.
...
- 가운데 칸 글자는 칸 크기에 맞춤(`JK_MID_FIT`). 「세트」 라벨 3배. 시각·경기 시계 세로는 2배·가로는 1.3배. 세트 점수 숫자는 크기 그대로 안쪽. 시각·새 경기는 세트 점수 아래 세로 가운데. 세트 칸에 팀 서브 문구 없음. 심플에서는 가운데 금색 서브 번호만 더 크게(`JK_SIMPLE`). 서브 팀 점수 숫자 바로 아래 짧은 막대(`JK_SERVE_BAR`, 흰 칸만 검정·나머지 흰색, 두께·길이 1.5배). 가운데 칸 폭은 그대로. 현재 시각·경기 시간(선서브부터, 새 경기에서 0). 새 경기 글자는 버튼에 맞춤
...
- 심플, TTS, BLE, 스킨, 하단 메뉴. 심플 코트 원 숨김, 가운데 서브 번호(화살표 없음)
