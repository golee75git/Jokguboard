...
- 패드·볼륨·미디어 키 중 같은 눌림이 두 경로로 들어와도 한 번만 반영(교차 입력 잠금). 위 버튼으로 판정된 뒤에만 시스템 바를 바로 숨기고, 직후 0.8초 동안 바가 다시 켜지면 나타난 즉시 숨김. 손가락으로 연 바는 그대로(`JK_HID_PAD_LOCK`, `JK_HID_BAR_GUARD`). 맞춤이 켜진 동안 3버튼 리모컨 마우스 커서는 숨김(`JK_PAD_POINTER_HIDE`)
