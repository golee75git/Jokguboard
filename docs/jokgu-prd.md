...
- 가운데 세트 칸 글자는 칸 크기에 맞춤. 세트 점수 숫자는 크기 그대로 안쪽으로. 시각·서브·새 경기는 세트 점수 아래 세로 가운데. 현재 시각·경기 시간(선서브부터). 새 경기 글자는 버튼에 맞춤.
- 경기는 3세트 2선승만 (`bo3_2`).
...
- 가운데 세트 칸: 「세트」 글자 3배, 시각·경기 시계 세로는 2배·가로는 1.3배. 세트 점수 숫자는 크기 그대로 안쪽, 시각·새 경기는 세트 점수 아래 세로 가운데. 세트 칸에 팀 서브 문구 없음. 경기 시간은 선서브부터, 새 경기에서 0.
- 설정: 종목, Futnet 유형, 언어, 글꼴, 보드 모습, 점수 읽기
- 심플 모드(선서브 후). 심플에서는 새 경기·코트 원 숨김. 가운데에 금색 서브 번호(칸 맨 위, 원 안 숫자 1.5배, 화살표 없음). 서브 팀 점수 숫자 바로 아래에 짧은 막대(밝은 백 칸 검정, 어두운 칸 흰색, 두께·길이 1.5배).
...
- 패드·볼륨·미디어 키 중 같은 눌림이 두 경로로 들어와도 한 번만 반영(교차 입력 잠금). 위 버튼으로 판정된 뒤에만 시스템 바를 바로 숨기고, OS가 늦게 켜면 짧은 간격으로 다시 숨김. 손가락으로 연 바는 그대로(`JK_HID_PAD_LOCK`, `JK_HID_BAR_GUARD`). 맞춤이 켜진 동안 3버튼 리모컨 마우스 커서는 숨김(`JK_PAD_POINTER_HIDE`). HID +1은 숫자만 바꾸고 커짐·+1 표식은 없음(`JK_HID_NO_BUMP`). 화면 터치 +1은 애니 유지
