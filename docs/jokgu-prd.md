...
- 가운데 세트 칸: 「세트」·시각·경기 시간은 칸 비율, 세트 점수 숫자는 크기 그대로 안쪽, 시각·새 경기는 세트 점수 아래 세로 가운데. 세트 칸에 팀 서브 문구 없음. 경기 시간은 선서브부터, 새 경기에서 0.
- 설정: 종목, Futnet 유형, 언어, 글꼴, 보드 모습, 점수 읽기
- 심플 모드(선서브 후). 심플에서는 새 경기·코트 원 숨김. 가운데에 금색 서브 번호(더 큼, 화살표 없음). 서브 팀 큰 점수 아래에 짧은 막대(어두운 칸 흰색, 밝은 칸 검은색).
